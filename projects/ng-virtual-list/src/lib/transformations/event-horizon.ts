...
/**
 * eventHorizon
 * @link https://github.com/DjonnyX/ng-virtual-list/blob/16.x/projects/ng-virtual-list/src/lib/transformations/event-horizon.ts
...
export const eventHorizon = (options?: IEventHorizonOptions): ItemTransform => {
    ...
    return (index: number, measures: IRenderVirtualListItemMeasures,
        ...
        if (config.snapped || config.snappedOut) {
            ...
            result.x = isVertical ? xx : (scrollSize - config.layoutOffset + boundsSizeHalf - itemSizeHalf + (xx * Math.pow(px, 2) * spacingBetweenItems));
            result.y = isVertical ? (scrollSize - config.layoutOffset + boundsSizeHalf - itemSizeHalf + (yy * Math.pow(py, 2) * spacingBetweenItems)) : yy;
