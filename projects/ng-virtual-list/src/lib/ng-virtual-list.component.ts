...
import {
  BehaviorSubject, combineLatest, debounceTime, delay, distinctUntilChanged, filter, fromEvent, map, of, Subject, switchMap, take, takeUntil, tap,
} from 'rxjs';
import {
  BEHAVIOR_INSTANT, CLASS_LIST_HORIZONTAL, CLASS_LIST_VERTICAL, DEFAULT_BUFFER_SIZE, DEFAULT_COLLAPSE_BY_CLICK, DEFAULT_DIRECTION,
...
export class NgVirtualListComponent extends DisposableComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  */
  ...
  private _$scrollToCanceller: Subject<void> | undefined = new Subject<void>();

  private cancelScrollTo() {
  ...
  constructor(
    ...
    const $container = this.$viewInitialized.pipe(
      filter(v => !!v),
      switchMap(() => of(this._container)),
    );
    ...
    const _$scrollCanceller = new BehaviorSubject<boolean>(false);
    ...
    $container.pipe(
      ...
      switchMap(container => {
        return fromEvent(container, SCROLL).pipe(
          filter(() => !_$scrollCanceller.getValue()),
        );
      }),
      tap(e => {
        this.cancelScrollTo();
        ...
        if (items && items.length) {
          ...
          if (dynamicSize) {
            ...
            if ((!notChanged && iteration < MAX_SCROLL_TO_ITERATIONS) || iteration < MAX_SCROLL_TO_ITERATIONS) {
              this.cancelScrollTo();
              this._$scrollToCanceller = new Subject<void>();
            ...
            if (index > -1) {
              ...
              if (currentScollSize !== scrollSize) {
                const params: ScrollToOptions = { [this._isVertical ? TOP_PROP_NAME : LEFT_PROP_NAME]: scrollSize, behavior: behavior as ScrollBehavior };
                cnt.scrollTo(params);
  ...
  scrollToEnd(cb?: () => void, options?: IScrollOptions) {
    ...
    validateScrollBehavior(behavior);
    validateIteration(iteration);
    const items = this.items, trackBy = this.trackBy, latItem = items[items.length > 0 ? items.length - 1 : 0], id = latItem[trackBy],
