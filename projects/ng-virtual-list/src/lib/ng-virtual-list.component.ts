 ...
 * @link https://github.com/DjonnyX/ng-virtual-list/blob/21.x/projects/ng-virtual-list/src/lib/ng-virtual-list.component.ts
...
export class NgVirtualListComponent implements OnDestroy {
  ...
  protected actualItemSize = signal<number>(this.itemSize());
  ...
  constructor() {
    ...
    combineLatest([$viewInit, $dynamicSize, $snapScrollToBottom, $waitForPreparation]).pipe(
      takeUntilDestroyed(this._destroyRef),
      distinctUntilChanged(),
      filter(([init]) => !!init),
    ...
    const $bounds = toObservable(this._bounds).pipe(
      ...
      $actualItemSize = toObservable(this.actualItemSize).pipe(
        map(v => v <= 0 ? DEFAULT_ITEM_SIZE : v),
      ),
