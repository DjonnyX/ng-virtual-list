 ...
 * @link https://github.com/DjonnyX/ng-virtual-list/blob/16.x/projects/ng-virtual-list/src/lib/utils/track-box.ts
            ...
            if (crudDetected) {
                ...
                for (let i = 0, l = collection.length; i < l; i++) {
                    ...
                    if (map.has(id)) {
                        const cache = map.get(id)!;
                        itemDisplayMethod = cache?.method ?? ItemDisplayMethods.UPDATE;
                        const isItemNew = (cache as Cache)?.[IS_NEW] ?? this._isLazy;
